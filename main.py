...
# ------------------------ MASTER CSV LOGIC ------------------------
def find_source_master_file(session, folder_id, source, study_type):
    """Find the current master file for the given source (ignoring date) in the given folder."""
    prefix = f"{study_type}_{source}_master_"
    entries = get_folder_entries(session, folder_id) or []
    for entry in entries:
...
def update_master_csv(session, group_row, question_row, data_row,
                      folder_id, source, study_type, formatted_date_str):
    """
    Update master CSV file in folder_id:
    ...
    """
    file_id, old_name = find_source_master_file(session, folder_id, source, study_type)
    ...
    if file_id:
        resp = session.post(BOX_UPDATE_URL.format(file_id=file_id),
                            files={'file': (old_name, csv_content, 'text/csv')})
        if resp.status_code in (200, 201):
            print("✅ Updated master content")
            if old_name != new_master_name and should_update_master_filename(old_name, formatted_date_str, study_type, source):
        ...
        if upload_file(session, new_master_name, csv_content, folder_id):
...
def process_master_file_update(session, data, questionnaire, folder_id,
                               group_row, question_row, data_row,
                               source, study_type, formatted_date_str):
    """
    Handle master CSV file update from the current (single questionnaire) data.
    For postScan we skip this path and rely on merged single_file data instead.
    """
    do_master = data.get("master", True)
    if not do_master:
...
def clean_pq16_data(response_data):
    """
    Clean PQ-16 data by clearing distressChoice and distressValue when symptomValue is 0.
    """
...
def merge_csvs_for_participant(session, folder_id, study_type, source, participant_id, formatted_date_str, entries,
    ...
    - Only triggered when questionnaire == "pq16".
    - Always append the merged, in-memory row to the master in the ROOT folder,
      even if the single_file upload returns 409 (already exists).
    """
    ...
    uploaded = upload_file(session, merged_filename, buf.getvalue(), single_file_folder_id)
    if not uploaded:
        print(f"ℹ️ {merged_filename} already exists in '{subfolder_name}' — still appending to master from in-memory data.")

    # Append to master in the ROOT folder regardless of upload result
    update_master_csv(
        session=session,
        group_row=merged_header,
        question_row=merged_label,
        data_row=merged_data,
        folder_id=root_folder_id,        # IMPORTANT: root folder (study folder), not the participant subfolder
        source=source,
        study_type=study_type,
        formatted_date_str=formatted_date_str
    )

    print(f"Horizontally merged CSV handled as {merged_filename} (uploaded={uploaded}) and appended to master")
    return True
...
def webhook():
    ...
    participant_id = (response_data.get("participantID") or "").strip() or "unknownID"
    ...
    # Check if there is any data other than date/time/participantID
    non_empty_fields = [
        f for f in fieldnames
        if f not in ("date", "time", "participantID") and str(all_response_data.get(f, "")).strip() != ""
    ]
    ...
    try:
        ...
        if source in SUBFOLDER_SOURCES:
            ...
            root_folder_id = folder_id  # save the root (study) folder to use for master and single_file
    ...
    individual_result = process_individual_file_upload(
    ...
    # Master file update:
    # For postScan we DO NOT append partial rows here; we rely on the merged single_file (pq16) step to append once.
    if source != "postScan":
        master_folder_id = root_folder_id or folder_id  # for non-postScan, this is the appropriate folder
        if process_master_file_update(
            session, data, questionnaire, master_folder_id,
            group_row, question_row, data_row,
            source, study_type, formatted_date_str
        ):
            success_count += 1

    # Use the global QUESTIONNAIRE_ORDER variable for merging (only triggers on pq16)
    subfolder_name = "single_file"
    if merge_csvs_for_participant(
        session, folder_id, study_type, source, participant_id, formatted_date_str, entries,
